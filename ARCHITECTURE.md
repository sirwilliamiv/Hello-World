...
Flagged rather than decided, because each is expensive to reverse and none blocks the
Phase 1 schemas.

1. **Multi-tenancy of the generated app.** `org.teams` extends *every tenant-scoped entity*
   with an organization reference. Row-level security in Postgres versus repository-layer
   scoping is a decision that touches every capability's queries. The catalog says
   "enforced at the repository layer, not by convention," which reads as a decision already
   made — this document assumes repository-layer scoping with a mandatory cross-tenant leak
   test per capability. **Confirm.**

2. **ORM.** Prisma, Drizzle, or hand-rolled. This determines how migrations are authored and
   whether capability schemas can be composed at all. Drizzle is the current inclination:
   its migrations are plain SQL (three-way mergeable, inspectable in a plan) and its schema
   is TypeScript that composes across packages, where Prisma's single-file `schema.prisma`
   would have to be *generated by merging every capability's schema* — a large, fragile
   managed file exactly where we least want one. **This is a Phase 1 blocker and should be
   decided at review.**

3. **Capability package distribution.** A private npm registry (Artifact Registry) versus a
   pnpm workspace with clients as a workspace member. The registry is more work but is what
   makes a client repository independent of ours.

4. **Where the client repository lives.** One GitHub repository per client under our
   organisation is assumed. It determines how `forge fleet apply` opens twenty pull
   requests, which is probably how fleet upgrades should actually land — as reviewable PRs
   rather than direct pushes.

5. **`integrate.custom` pricing and weight.** The schema currently models price as
   `{ model: "quoted" }` with no amount and weight as a range. Confirm that a quoted
   capability should participate in the automatic weight computation at all.

6. **Contract test residency.** Contract tests are defined by declaration pairs, so they can
   live in the catalog and run against a synthetic product, or be generated into each
   client repository and run in that client's CI. Catalog-side is cheaper and catches
   regressions before release; client-side catches client-specific misconfiguration. The
   current position is catalog-side for capability pairs and client-side for smoke tests
   only. **Confirm.**
